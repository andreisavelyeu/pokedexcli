...
func NewCache() Cache {
	return Cache{Data: make(map[string]CacheEntry), M: sync.Mutex{}, Interval: time.Duration(5 * time.Minute)}
}
...
func (p *Pokedex) Add(pokemon PokemonResponse) {
	p.Caught[pokemon.Name] = pokemon
}

func NewPokedex() *Pokedex {
	return &Pokedex{Caught: make(map[string]PokemonResponse)}
}
